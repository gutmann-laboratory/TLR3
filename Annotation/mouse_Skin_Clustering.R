...
name_list <- dir(paste0(mouse_data_path, "Skin_GSE121478/"))

SO_list <- lapply(name_list, function(x){
    data <- Read10X(data.dir = paste0(mouse_data_path, "Skin_GSE121478/", x))
...
Clusters$celltype_broad <- ifelse(Clusters$seurat_clusters %in% c(1, 2, 4, 5), "T/NK cell",
                                     ifelse(Clusters$seurat_clusters %in% c(0, 3, 6, 7, 8, 9, 10), "TAM",
                                            "Others"))

Clusters$tissue <- "Skin"
Clusters$tumor <- "Tumor"
Clusters$ID <- ifelse(Clusters$orig.ident == "Tumor_1", "Tumor1_Skin", "Tumor2_Skin")
