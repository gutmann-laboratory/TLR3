...
name_list <- dir(paste0(data_path, "/Pancreas_GSE214295/")) 
...
Clusters$celltype_broad <- ifelse(Clusters$seurat_clusters %in% c(1), "T/NK cell",
                                     ifelse(Clusters$seurat_clusters %in% c(5), "TAM",
                                           ifelse(Clusters$seurat_clusters %in% c(0, 3, 8), "Ductal cell", #Tumor
                                                 ifelse(Clusters$seurat_clusters %in% c(4), "Endothelial cell",
                                                       ifelse(Clusters$seurat_clusters %in% c(9), "B/Plasma cell",
                                                             ifelse(Clusters$seurat_clusters %in% c(2), "Fibroblast",
                                                                    ifelse(Clusters$seurat_clusters %in% c(6), "Pericyte",
                                                                   ifelse(Clusters$seurat_clusters %in% c(7), "Mast cell",
                                                                          ifelse(Clusters$seurat_clusters %in% c(10), "Acinar cell",
                                                                                 ifelse(Clusters$seurat_clusters %in% c(11), "Schwann cell",
                                                                                        ifelse(Clusters$seurat_clusters %in% c(12), "β-cells","Others")))))))))))

Clusters_sub$tissue <- "Pancreas"
Clusters_sub$tumor <- "Tumor"

Clusters$ID <- ifelse(Clusters$orig.ident == "Primary001", "Tumor_Pancreas1",
                      ifelse(Clusters$orig.ident == "Primary002", "Tumor_Pancreas2",
                             ifelse(Clusters$orig.ident == "Primary003", "Tumor_Pancreas3", "Others")))
