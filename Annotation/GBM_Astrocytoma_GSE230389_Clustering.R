...
# remove Cluster1 (nFeature < 300)
Clusters_sub <- subset(Clusters, seurat_clusters %in% c(0, 2:7))

Clusters_sub$celltype_broad <- ifelse(Clusters_sub$seurat_clusters %in% c(5), "TAM",
                                     ifelse(Clusters_sub$seurat_clusters %in% c(6), "Oligodendrocyte",
                                           ifelse(Clusters_sub$seurat_clusters %in% c(2, 3), "Tumor",
                                                 ifelse(Clusters_sub$seurat_clusters %in% c(4), "Tumor", "Others"))))

Clusters_sub$tissue <- "Brain"
Clusters_sub$tumor <- "Tumor"

Clusters_sub$ID <- ifelse(Clusters_sub$orig.ident == "Astrocytoma", "Tumor_Brain_LGG_Astrocytoma", "Tumor_Brain_HGG_GBM")
table(Clusters_sub$ID)
